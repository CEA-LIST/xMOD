...
model_path = './tmp/train_bg_control_0.3regL1_test_wo_graph_0.5_weight_moving_final_setting_dinov2_wo_clip/epoch_450.ckpt'
data_path = './test_video'
...
sample = test_set[0]
images = sample['image']
# to_display = sample['mask'][:5]
# print(to_display.shape)

# for i in range (5):
#     plt.imsave('./infer/epoch410_moving/image{}.png'.format(i), to_display[i,:,:].detach().cpu().numpy())
...
for j in range(5):
    ...
    fig.savefig('./infer/epoch450_moving/frame_dinov2_-{}.png'.format(j))
    plt.close(fig)
