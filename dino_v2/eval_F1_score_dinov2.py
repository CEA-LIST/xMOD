...
parser.add_argument('--ckpt_path', default='./tmp/train_bg_control_0.2regL1_test_wo_graph_1.0_weight_est_final_setting_w_clip1_f50_mean_accross_batch_correct_wbce_batch8_param_reg_on_bg_dinov2/epoch_310.ckpt', type=str, help='pre-trained model path' )

parser.add_argument('--test_path', default = './test_video', type = str, help = 'path of PD test set')
...
def main():
    ...
    # test_set_est = PDDataset(split = 'eval', root = data_path, gt='moving_masks')
    ...
    for  sample in tqdm(test_dataloader):
        ...
        mask_gts = F.interpolate(mask_gt.float(), (120, 242)).long() #TRI-PD
        # mask_gts = F.interpolate(mask_gt.float(), (123, 245)).long() #dino_v2

        if opt.is_bg_handled:   
            for i in range(40):    # change to 40         
                _, masks, _ , _= model(image[:,i*5:i*5+5,:,:,:])
                mask_gt = mask_gts[:,i*5:i*5+5,:,:] # uncomment for TRI-PD
                # mask_gt = mask_gts 
                masks = masks.detach().cpu()

                for fm in range(5): # change to 5
                    ...
                    # active_slots.append([gt_msk[:,:,s] for s in range(1, gt_msk.shape[2]) if gt_msk[:,:,s].max().item()>0]) # to check only, reset previous


                    scores.append([masks[0,fm,s,:,:][pred_msk[:,:,s] == 1].mean() for s in range(opt.num_slots-1) if pred_msk[:,:,s].max().item()>0])
                    # scores.append([1 for s in range(len(active_slots[-1])) ]) # to check only, reset previous
    ...
    rec, prec, ap = voc_eval(all_gt, scores, active_slots, 0.5)
    print('final AP:',ap)
