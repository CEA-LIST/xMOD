...
from  ..dataset.datasetKITTIEval_dinov2 import KITTIDataset
from eval.metrics import ARI 
import matplotlib.pyplot as plt
from models.model_bg_dinov2_KITTI import *
...
parser.add_argument('--ckpt_path', default='./tmp/trainKITTI_ts_122_dinov2_correct/epoch_100.ckpt', type=str, help='pre-trained model path' )


parser.add_argument('--test_path', default = './KITTI_DOM/KITTI_DOM_test', type = str, help = 'path of PD test set')
    ...
    for  sample in tqdm(test_dataloader):
        if opt.is_bg_handled: 
            ...
            for i in range(8):
                ...
                # plt.imsave('debug/check_gt_KITTI/{}_{}.png'.format(nb_samples, i), 255*mask_gt[i,0,:,:].numpy()/(mask_gt[i,0,:,:].numpy().max()))
                gt_msk = F.one_hot(mask_gt[i,0,:,:])
                pred_msk = F.one_hot(masks[i,0,:,:,:].argmax(dim=0, keepdim=False))
                
                print('gt_msk', gt_msk.shape)
