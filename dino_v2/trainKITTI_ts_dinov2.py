...
from  ..dataset.datasetKITTIEval_dinov2 import KITTIDataset as KITTIDatasetEval

from  models.model_bg_dinov2 import SlotAttentionAutoEncoder
from transforms.transforms_2d import get_normalize_transform, get_ssl_train_online_transform, get_crop_transform
from eval.metrics import ARI 
from train.utils import temporal_loss as t_loss 
...
parser.add_argument('--exp_name', default='trainKITTI_ts_122_dinov2_correct', type=str, help='experiment name, used for model saving/plotting/wand ect' )
...
parser.add_argument('--data_path', default = './KITTI_DOM/KITTI_DOM_train/', type = str, help = 'path of PD dataset')
...
parser.add_argument('--entity', default='', type = str, help = 'wandb name')
# Data augmentations
            ...
            for idx, s in enumerate(id_slot):
                ...
                if not teacher:  
                    ...
                    if weight>=0.9:
                        nb_objects += 1
                        tmp_BCE += ( -(1+weight)*torch.log(masks[i,j,s,:,:]) * mask_gt[i,j,id_gt[idx],:,:] - (1- mask_gt[i,j,id_gt[idx],:,:]) * torch.log(1-masks[i,j,s,:,:])).mean()
                        # plt.imsave('debug/{}_{}_{}_weight{}.png'.format(i,j,idx,weight), mask_gt[i,j,id_gt[idx],:,:].detach().cpu().numpy()*255)

            # print('nb_objects', nb_objects)
            if nb_objects == 0:
                continue
            tmp_BCE /= nb_objects
            loss_mask += tmp_BCE
...
def main():
    ...
    # test_set = PDDataset(split = 'eval', root = data_path, supervision = opt.supervision, transform=None, apply_img_transform=False)
    ...
    step = 0
    best_ckpt = 490
    burn_in_exp = 'trainPD_ts_burnIn_400_reg_0.3_final_dinov2'
    ...
    for epoch in range(opt.num_epochs):
        ...
        if epoch == opt.start_teacher:
            print('____________________best_ckpt', best_ckpt)
            print('____________________burn_in_exp', burn_in_exp)
            print('____________________supervision', opt.supervision)
            print('____________________regularization', opt.weight_reg)
        
            model_path = os.path.join(opt.model_dir, burn_in_exp, 'epoch_{}.ckpt'.format(best_ckpt))
        ...
        for sample in tqdm(train_dataloader):
            ...
            # weighted BCE
            if epoch >= opt.start_teacher: #lately changed >=
        ...
        if not epoch % 10:
            ...
            # image = val_transform(image).view(img_shape)
            ...
            if opt.wandb:
                wandb.log({"recon_loss": total_loss,  'mask_loss': M_loss, 'L1_reg': LR_loss})
        ...
        if not epoch % 10:
            ...
            #         mask_gts = F.interpolate(mask_gt.float(), (95, 315)).long()
