...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        # (27,90) are the spatial dims after passing bigger image into ViT-S14
        x = x.permute(0,2,1)
        x = x.view(bs*n_frames, x.shape[1], ceil(H/14), ceil(W/14))   # (27, 90)
        x = F.interpolate(x, (ceil(H/4), ceil(W/4))) # (95,315)
       
        x = x.view(bs, n_frames, x.shape[1], ceil(H/4), ceil(W/4))  
        x,_ = self.convGRU(x)
        x = x[0]
...
class Decoder(nn.Module):
    def __init__(self, hid_dim, resolution, output_channel):
        ...
        self.resolution = resolution
        # self.dinov2_resolution = (490, 980)
